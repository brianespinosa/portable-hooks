...
Keep our class state/methods in mind 👀
...
- Encapsulate state logic
- Return state and setter methods
- Do NOT return state setter directly ⚠️
...
- Challenge yourself/team to compare APIs
- Learn from prior art (Reakit, Ariakit, etc.)
- Talk to colleagues who have used these patterns ([#portable-hooks](https://surveymonkey.slack.com/archives/C0442AHFGUU))
...
export default function App() {
  ...
  return (
    <>
      ...
      <button onClick={handleCreateTag}>Franco</button>
      <button onClick={handleCreateTag}>Pogo</button>
      {/* Can't separate the input from the tags */}
      <Tag {...tagStateE} />
    </>
  );
}
...
```jsx src/Tag.js focus=11:21,25:33
import React, { useState } from 'react';
import { AnimatePresence, m } from 'framer-motion';
...
export default function TagInput({ addTag } = {}) {
  const [newLabel, setNewLabel] = useState('');
...
TagInput.displayName = 'Tag.Input';
```

---

```jsx src/Tag.js focus=34:55
import React, { useState } from 'react';
import { AnimatePresence, m } from 'framer-motion';
...
```jsx src/Tag.js focus=13:23,28:35
import React, { useState } from 'react';
import { AnimatePresence, m } from 'framer-motion';
import TagInput from './TagInput';
import TagTags from './TagTags';
...
export default function App() {
  ...
  return (
    <>
      <h2>Declarative Subcomponents</h2>
      <button onClick={handleCreateTag}>Rocket</button>
      <button onClick={handleCreateTag}>Franco</button>
      <button onClick={handleCreateTag}>Pogo</button>
      <Tag>
